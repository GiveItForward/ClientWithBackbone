/**************************************************************************
 * svgavatars.core.min.js - jQuery script for creating vector avatars
...
//Global options
var path_to_folder = '/',//the path to main SVG Avatars folder from root dir of your site. The slashes "/" are important!
	...
	hide_save = false, //true will disable save on your server option
	...
	hide_png_one_download = true, //true will disable download PNG with first dimensions
	hide_png_two_download = true, //true will disable download PNG with second dimensions
	hide_gravatar = true, //true will disable the possibility to install created avatar as gravatar
	color_theme = 'light'; //must be exactly 'light' or 'dark'

//Share options
var	hide_share = true, //true will disable share option
	...
	share_credit = 'Created on giveitforward.us';//replase YourSite.com with yours or leave it blank (do NOT delete variable itself!), if you don't want a watermark on avatar for Social share
...
SVG.extend(SVG.Element,{svgaCenterScale:function(f,b){var g=this.bbox(),m=g.cx,g=g.cy;b||(b=f);return this.transform({a:f,b:0,c:0,d:b,e:m-f*m,f:g-b*g})},svgaLeft:function(f,b){f=f?f:3;b=b?b:lr_step;var g=this.data("leftright"),m=this.data("updown");g>-(f*b)&&(this.move(g-b,m),this.data("leftright",g-b-1E-7));return this},svgaRight:function(f,b){f=f?f:3;b=b?b:lr_step;var g=this.data("leftright"),m=this.data("updown");g<f*b&&(this.move(g+b,m),this.data("leftright",g+b+1E-7));return this},svgaUp:function(f,
b){f=f?f:3;b=b?b:ud_step;var g=this.data("leftright"),m=this.data("updown");m>-(f*b)&&(this.move(g,m-b),this.data("updown",m-b-1E-7));return this},svgaDown:function(f,b){f=f?f:3;b=b?b:ud_step;var g=this.data("leftright"),m=this.data("updown");m<f*b&&(this.move(g,m+b),this.data("updown",m+b+1E-7));return this},svgaScaleUp:function(f,b,g){f=f?f:3;b=b?b:scale_step;g=g?g:b;var m=this.data("scaleX")+1E-7,r=this.data("scaleY")+1E-7;m<1+f*b&&(this.svgaCenterScale(m+b,r+g),this.data("scaleX",m+b+1.1E-7),
this.data("scaleY",r+g+1.1E-7));return this},svgaScaleDown:function(f,b,g){f=f?f:3;b=b?b:scale_step;g=g?g:b;var m=this.data("scaleX")-1E-7,r=this.data("scaleY")-1E-7;m>1-f*b&&(this.svgaCenterScale(m-b,r-g),this.data("scaleX",m-b-1.1E-7),this.data("scaleY",r-g-1.1E-7));return this},svgaRotateLeft:function(f,b,g,m){f=f?f:2;b=b?b:rotate_step;var r=this.data("rotate");r>-(f*b)&&(this.rotate(r-b-1E-7,g,m),this.data("rotate",r-b));return this},svgaRotateRight:function(f,b,g,m){f=f?f:2;b=b?b:rotate_step;
var r=this.data("rotate");r<f*b&&(this.rotate(r+b+1E-7,g,m),this.data("rotate",r+b));return this},svgaCancelRotate:function(){return this.rotate(1E-7).data("rotate",1E-7,!0)}});
...
$("#svga-start-boys").click(function(){$("#svga-gender").hide();setTimeout(function(){$.ajax({url:path_to_folder+"json/svgavatars-male-data.json",async:!1,dataType:"json",success:function(f){svgAvatars("boys",f);$("#svga-start-overlay").hide()},error:function(){$("#svga-message-text").html(alert_json_error).addClass("svga-error");$("#svga-loader").hide();$(".svga-close").hide();$("#svga-work-overlay").fadeIn("fast");$("#svga-message").fadeIn("fast")}})},50)});
$("#svga-start-girls").click(function(){$("#svga-gender").hide();setTimeout(function(){$.ajax({url:path_to_folder+"json/svgavatars-female-data.json",async:!1,dataType:"json",success:function(f){svgAvatars("girls",f);$("#svga-start-overlay").hide()},error:function(){$("#svga-message-text").html(alert_json_error).addClass("svga-error");$("#svga-loader").hide();$(".svga-close").hide();$("#svga-work-overlay").fadeIn("fast");$("#svga-message").fadeIn("fast")}})},50)});
function svgAvatars(f,b){function g(){function a(a){for(var c=0;c<N.length;c++)N[c].match(/scale/)?a.data(N[c],1,!0):a.data(N[c],0,!0)}l=SVG("svga-svgmain").attr({id:"svga-svgcanvas",width:null,height:null,"class":"svga-svg",viewBox:"0 0 200 200",preserveAspectRatio:"xMinYMin meet"});l=l.group().attr("id","svga-group-wrapper");d=l.group().attr("id","svga-group-backs-single");l=l.group().attr("id","svga-group-subwrapper");a(l);d=l.group().attr("id","svga-group-hair-back");a(d);d=l.group().attr("id",
"svga-group-humanbody-single");d=l.group().attr("id","svga-group-chinshadow-single");d=l.group().attr("id","svga-group-clothes-single");a(d);v=l.group().attr("id","svga-group-head");a(v);d=l.group().attr("id","svga-group-ears-left");a(d);v.add(d);d=l.group().attr("id","svga-group-ears-right");a(d);v.add(d);d=l.group().attr("id","svga-group-faceshape-wrap");z=l.group().attr("id","svga-group-faceshape-single");d.add(z);a(d);a(z);v.add(d);d=l.group().attr("id","svga-group-mouth-single");a(d);v.add(d);
d=l.group().attr("id","svga-group-eyes-left");z=l.group().attr("id","svga-group-eyesback-left");J=l.group().attr("id","svga-group-eyesiriswrapper-left");C=l.group().attr("id","svga-group-eyesiriscontrol-left");K=l.group().attr("id","svga-group-eyesiris-left");O=l.group().attr("id","svga-group-eyesfront-left");J.add(C);C.add(K);d.add(z).add(J).add(O);a(d);a(C);a(K);v.add(d);d=l.group().attr("id","svga-group-eyes-right");z=l.group().attr("id","svga-group-eyesback-right");J=l.group().attr("id","svga-group-eyesiriswrapper-right");
C=l.group().attr("id","svga-group-eyesiriscontrol-right");K=l.group().attr("id","svga-group-eyesiris-right");O=l.group().attr("id","svga-group-eyesfront-right");J.add(C);C.add(K);d.add(z).add(J).add(O);a(d);a(C);a(K);v.add(d);d=l.group().attr("id","svga-group-facehighlight-single");a(d);v.add(d);d=l.group().attr("id","svga-group-eyebrows-left");a(d);v.add(d);d=l.group().attr("id","svga-group-eyebrows-right");a(d);v.add(d);d=l.group().attr("id","svga-group-nose-single");a(d);v.add(d);"boys"===f&&(d=
l.group().attr("id","svga-group-beardwrap"),z=l.group().attr("id","svga-group-beard-single"),d.add(z),a(d),a(z),v.add(d),d=l.group().attr("id","svga-group-mustache-single"),a(d),v.add(d));d=l.group().attr("id","svga-group-hair-front");a(d);v.add(d);d=l.group().attr("id","svga-group-glasses-single");a(d);v.add(d);if(share_credit){l=SVG.get("svga-group-wrapper");s=l.rect(200,15).move(0,185).fill("#ecf0f1").opacity(0.5);q=l.text(share_credit).font({family:'"Roboto", "Helvetica Neue", Helvetica, Arial, sans-serif',
size:"0.84em",anchor:"middle",weight:400}).fill("#000");var c=q.bbox().width;q.move((200-c)/2,185);s.hide();q.hide()}}function m(a,c,f,d){for(var l=b[c].shapes[f][d],g=0;g<l.length;g++){var h=l[g],k=a.path(h.path,!0);k.data("colored",h.colored,!0);k.data("transparence",h.transparence,!0);k.data("filltype",h.fill);k.data("stroketype",h.stroke);h.fromskin&&k.data("fromskin",h.fromskin,!0);if(!0===k.data("colored")){var e=h.fromskin?"faceshape":c,m=k.data("filltype"),n=!1,m=r(m,e,n);k.attr("fill",m);
m=k.data("stroketype");e=r(m,e,n);k.attr({stroke:e,"stroke-width":h.strokeWidth,"stroke-linecap":h.strokeLinecap,"stroke-linejoin":h.strokeLinejoin,"stroke-miterlimit":h.strokeMiterlimit})}else"gradient"===h.fill?(H?Y("svga-on-canvas-"+c+"-gradient-"+d+"-"+g):(e=document.getElementById("svga-"+c+"-gradient-"+d+"-"+f+"-"+g))&&e.parentNode.removeChild(e),e=a.gradient(h.type,function(a){for(var b=0;b<h.gradientStops.length;b++){var f=h.gradientStops[b].color;n=!0;var d=r(f,h.fromskin?"faceshape":c,n),
e=a.at(h.gradientStops[b]);e.update({color:d});e.data("stoptype",f)}}),h.x1&&e.attr({x1:h.x1}),h.y1&&e.attr({y1:h.y1}),h.x2&&e.attr({x2:h.x2}),h.y2&&e.attr({y2:h.y2}),h.cx&&e.attr({cx:h.cx}),h.cy&&e.attr({cy:h.cy}),h.fx&&e.attr({fx:h.fx}),h.fy&&e.attr({fy:h.fy}),h.r&&e.attr({r:h.r}),h.gradientTransform&&e.attr({gradientTransform:h.gradientTransform}),h.gradientUnits&&e.attr({gradientUnits:h.gradientUnits}),H?e.attr("class","svga-on-canvas-"+c+"-gradient-"+d+"-"+g):e.attr("id","svga-"+c+"-gradient-"+
d+"-"+f+"-"+g),k.attr({fill:e})):(k.attr({fill:h.fill}),k.attr({stroke:h.stroke,"stroke-width":h.strokeWidth,"stroke-linecap":h.strokeLinecap,"stroke-linejoin":h.strokeLinejoin,"stroke-miterlimit":h.strokeMiterlimit}));h.opacity&&k.attr({opacity:h.opacity});H&&(h.id&&k.attr("id",h.id+"-"+d),"eyesback"==c&&SVG.get("svga-group-eyesiriswrapper-"+d).transform({x:0,y:0}),(h.irisx||h.irisy)&&SVG.get("svga-group-eyesiriswrapper-"+d).move(h.irisx,h.irisy),"hair"===c&&h.hideears?(SVG.get("svga-group-ears-left").hide(),
SVG.get("svga-group-ears-right").hide()):"hair"===c&&(SVG.get("svga-group-ears-left").show(),SVG.get("svga-group-ears-right").show()));!H&&h.hideonthumbs&&k.remove();H&&h.hideoncanvas&&k.remove()}}function r(a,c,b){switch(a){case "none":c="none";break;case "tone":c=w[c];break;case "hl05":c=D(w[c],-0.5*delta_sat,0.5*delta_val);break;case "hl1":c=D(w[c],-delta_sat,delta_val);break;case "hl2":c=D(w[c],-2*delta_sat,2*delta_val);break;case "sd05":c=D(w[c],0.5*delta_sat,-0.5*delta_val);break;case "sd1":c=
D(w[c],delta_sat,-delta_val);break;case "sd2":c=D(w[c],2*delta_sat,-2*delta_val);break;case "sd3":c=D(w[c],3*delta_sat,-3*delta_val);break;case "sd35":c=D(w[c],3.5*delta_sat,-3.5*delta_val);break;default:c=w[c],b&&(c=a)}return c}function E(a,c){for(var f=a.length,e=0;e<f;e++){1<f&&"mouth"!==a[e]&&"eyesfront"!==a[e]?w[a[e]]=c:w[p]=c;var d="facehighlight"===a[e]||"humanbody"===a[e]?0:M[a[e]];for(t in b[a[e]].shapes[d])b[a[e]].shapes[d].hasOwnProperty(t)&&SVG.get("svga-group-"+a[e]+"-"+t).each(function(c,
b){if(this.data("colored")){var f=!0==this.data("fromskin")?"faceshape":a[e],d=this.data("filltype"),d=r(d,f,!1);this.attr("fill",d);d=this.data("stroketype");d=r(d,f,!1);this.stroke(d)}else"gradient"==this.data("filltype")&&(d=this.attr("fill"),d=d.replace(/url\(\#/,"").replace(/\)/,""),d=SVG.get(d))&&(f=!0===this.data("fromskin")?"faceshape":a[e],d.each(function(a,c){var b=this.data("stoptype"),b=r(b,f,!0);this.update({color:b})}))})}}function V(a){a=tinycolor(a).toHexString();switch(p){case "faceshape":var c=
"faceshape humanbody chinshadow facehighlight ears mouth nose eyesfront".split(" ");E(c,a);break;case "ears":c="faceshape humanbody chinshadow facehighlight ears mouth nose eyesfront".split(" ");E(c,a);break;case "nose":c="faceshape humanbody chinshadow facehighlight ears mouth nose eyesfront".split(" ");E(c,a);break;default:c=[p],E(c,a)}}function S(){w="boys"===f?{backs:"#ecf0f1",humanbody:"#f0c7b1",clothes:"#386e77",hair:"#2a232b",ears:"#f0c7b1",faceshape:"#f0c7b1",chinshadow:"#f0c7b1",facehighlight:"#f0c7b1",
eyebrows:"#2a232b",eyesback:"#000000",eyesfront:"#000000",eyesiris:"#4e60a3",glasses:"#26120B",mustache:"#2a232b",beard:"#2a232b",mouth:"#da7c87"}:{backs:"#ecf0f1",humanbody:"#F3D4CF",clothes:"#09aac5",hair:"#2a232b",ears:"#F3D4CF",faceshape:"#F3D4CF",chinshadow:"#F3D4CF",facehighlight:"#F3D4CF",eyebrows:"#2a232b",eyesback:"#000000",eyesfront:"#000000",eyesiris:"#4e60a3",glasses:"#26120B",mouth:"#f771a9"};$("#svga-svgmain").empty();g();for(var a in b)u="backs"===a||"hair"===a?1:0,b.hasOwnProperty(a)&&
$("#svga-elements-"+a+"-"+u).trigger("click");$("#svga-colors-faceshape > div:nth-child(1)").trigger("click");k=2;P=!1}function W(){S();var a,c=x(0,19);if(1<x(0,2))if(0===x(0,1))var d=!0,e=!1;else d=!1,e=!0;for(var g in b){var l=!1;if(b.hasOwnProperty(g)){switch(g){case "ears":l=x(0,6);break;case "eyesiris":l=x(0,7);break;case "hair":l="boys"===f?x(0,17):x(0,14);break;case "mustache":d&&(l=x(1,12));break;case "beard":e&&(l=x(1,12));break;case "glasses":1<x(0,2)&&(l=x(0,17));break;default:l=x(0,14)}a=
x(0,19);l&&("hair"===g||"mustache"===g||"beard"===g||"eyebrows"===g?($("#svga-elements-"+g+"-"+l).trigger("click"),p=g,$("#svga-colors-"+p+" > div:nth-child("+c+")").trigger("click")):($("#svga-elements-"+g+"-"+l).trigger("click"),p=g,$("#svga-colors-"+p+" > div:nth-child("+a+")").trigger("click")))}}$("#svga-blocks-face").trigger("click");$("#svga-bodyzones-faceshape").trigger("click");k=2;P=!0}function I(a){$("#svga-loader").hide();$("#svga-gravatar-message-text").html(a).addClass("svga-error");
$("#svga-work-overlay").fadeIn("fast");$("#svga-gravatar-message").fadeIn("fast")}function Q(a,c){(new function(a){this.url=a;this.fileRequest=function(c,b){if(a){var d=document.createElement("input");d.setAttribute("name","filename");d.setAttribute("value",c);var e=document.createElement("input");e.setAttribute("name","filetype");e.setAttribute("value",b);var f=document.createElement("form");f.method="get";f.action=a;f.target="_top";f.appendChild(d);f.appendChild(e);document.body.appendChild(f);
f.submit();document.body.removeChild(f)}}}(path_to_folder+"php/download-temp-avatar.php")).fileRequest(a,c)}function Y(a){for(a=document.getElementsByClassName(a);0<a.length;)a[0].parentNode.removeChild(a[0])}function D(a,c,b){a=tinycolor(a).toHsv();a.s+=c;0>a.s&&(a.s=0);1<a.s&&(a.s=1);a.v+=b;0>a.v&&(a.v=0);1<a.v&&(a.v=1);return tinycolor(a).toHexString()}function x(a,c){return Math.floor(Math.random()*(c-a+1))+a}if("boys"===f){var w={backs:"#ecf0f1",humanbody:"#f0c7b1",clothes:"#386e77",hair:"#2a232b",
ears:"#f0c7b1",faceshape:"#f0c7b1",chinshadow:"#f0c7b1",facehighlight:"#f0c7b1",eyebrows:"#2a232b",eyesback:"#000000",eyesfront:"#000000",eyesiris:"#4e60a3",glasses:"#26120B",mustache:"#2a232b",beard:"#2a232b",mouth:"#da7c87"};$("#svga-container").addClass("svga-boys")}else if("girls"===f)w={backs:"#ecf0f1",humanbody:"#F3D4CF",clothes:"#09aac5",hair:"#2a232b",ears:"#F3D4CF",faceshape:"#F3D4CF",chinshadow:"#F3D4CF",facehighlight:"#F3D4CF",eyebrows:"#2a232b",eyesback:"#000000",eyesfront:"#000000",eyesiris:"#4e60a3",
glasses:"#26120B",mouth:"#f771a9"},$("#svga-container").addClass("svga-girls");else return;var F=["face","eyes","hair","clothes","backs"],G="boys"===f?"backs faceshape chinshadow facehighlight humanbody clothes hair ears eyebrows eyesback eyesiris eyesfront glasses mouth mustache beard nose".split(" "):"backs faceshape chinshadow facehighlight humanbody clothes hair ears eyebrows eyesback eyesiris eyesfront glasses mouth nose".split(" "),N="updown leftright dist scaleX scaleY rotate".split(" "),M=
{},y={up:"M8.425,3.176c-0.235-0.234-0.614-0.234-0.849,0L2.769,7.984c-0.235,0.234-0.235,0.613,0,0.85l0.565,0.564c0.234,0.235,0.614,0.235,0.849,0L7,6.58V12.4C7,12.732,7.268,13,7.6,13H8.4C8.731,13,9,12.73,9,12.4V6.58l2.818,2.819c0.234,0.234,0.614,0.234,0.849,0l0.565-0.566c0.234-0.234,0.234-0.613,0-0.848L8.425,3.176z",down:"M7.575,12.824c0.235,0.234,0.614,0.234,0.849,0l4.808-4.809c0.235-0.234,0.235-0.613,0-0.85l-0.565-0.564c-0.234-0.235-0.614-0.235-0.849,0L9,9.42V3.6C9,3.268,8.732,3,8.4,3H7.6C7.269,3,7,3.27,7,3.6v5.82L4.182,6.602c-0.234-0.234-0.615-0.234-0.849,0L2.768,7.168c-0.234,0.234-0.234,0.613,0,0.848L7.575,12.824z",
...
pinterest:"M27 2H5C3.34 2 2 3.34 2 5v22c0 1.66 1.34 3 3 3h22c1.66 0 3-1.34 3-3V5C30 3.34 28.66 2 27 2zM17.36 20.23c-1.28-0.1-1.81-0.73-2.82-1.34C14 21.78 13.32 24.55 11.33 26c-0.62-4.37 0.9-7.65 1.61-11.13 -1.2-2.03 0.15-6.1 2.68-5.1 3.12 1.24-2.71 7.53 1.21 8.32 4.09 0.82 5.75-7.09 3.22-9.66 -3.66-3.71-10.66-0.08-9.8 5.23 0.21 1.3 1.55 1.69 0.54 3.49C5.64 16 7.18 5.94 14.95 5.07 27.93 3.62 25.78 20.89 17.36 20.23L17.36 20.23z"},A="up down left right tightly wider scaledown scaleup eb1 eb2 eb3 eb4 ebcancel".split(" "),
L="up down left right scaledown scaleup tiltleft tiltright".split(" "),T="random reset save share gravatar download".split(" "),U=["facebook","twitter","pinterest","googleplus"],Z=["#385997","#19A6CA","#CB2028","#D33D2C"],X={backs:"backs",face:"faceshape",eyes:"eyesfront",hair:"hair",clothes:"clothes"},k=1,H=!1,p="faceshape",R,P=!1,d,z,J,C,K,O,l,v,s,q;g();for(var e=0;e<F.length;e++)$("#svga-blocks").append('<div id="svga-blocks-'+F[e]+'" class="svga-blocks" data-blockname="'+F[e]+'">'+block_titles[F[e]]+
"</div>");$(".svga-blocks:last").addClass("svga-last");$("#svga-blocks-backs").data("bodyzones","backs");$("#svga-blocks-face").data("bodyzones","faceshape,nose,mouth,ears");$("#svga-blocks-eyes").data("bodyzones","eyesfront,eyesiris,eyebrows,glasses");"boys"===f?$("#svga-blocks-hair").data("bodyzones","hair,mustache,beard"):$("#svga-blocks-hair").data("bodyzones","hair");$("#svga-blocks-clothes").data("bodyzones","clothes");for(var n in b)b.hasOwnProperty(n)&&($("#svga-bodyzones").append('<div id="svga-bodyzones-'+
n+'" class="svga-bodyzones" data-bodyzone="'+n+'" data-controls="'+b[n].controls+'" data-block="'+b[n].block+'">'+body_zone_titles[n]+"</div>"),$("#svga-bodyzones-"+n).hide());for(var B in y)y.hasOwnProperty(B)&&-1<A.indexOf(B)&&($("#svga-controls").append('<div id="svga-controls-'+B+'" class="svga-controls"><svg class="svga-control-icon" xmlns="http://www.w3.org/2000/svg" version="1.1" xlink="http://www.w3.org/1999/xlink" viewBox="0 0 16 16" preserveAspectRatio="xMinYMin meet"><path class="svga-control-icon-path" d="'+
y[B]+'"/></svg></div>'),$("#svga-controls-"+B).hide());for(B in y)y.hasOwnProperty(B)&&-1<L.indexOf(B)&&$("#svga-glob-controls").append('<div id="svga-glob-controls-'+B+'" class="svga-glob-controls"><svg class="svga-control-icon" xmlns="http://www.w3.org/2000/svg" version="1.1" xlink="http://www.w3.org/1999/xlink" viewBox="0 0 16 16" preserveAspectRatio="xMinYMin meet"><path class="svga-control-icon-path" d="'+y[B]+'"/></svg></div>');for(e=0;e<T.length;e++)$("#svga-"+T[e]+"avatar > div").append('<svg class="svga-menu-icon" xmlns="http://www.w3.org/2000/svg" version="1.1" xlink="http://www.w3.org/1999/xlink" viewBox="0 0 28 28" preserveAspectRatio="xMinYMin meet"><path class="svga-menu-icon-path" d="'+
y[T[e]]+'"/></svg>');for(e=0;e<U.length;e++)$("#svga-"+U[e]+"-icon").append('<svg class="svga-share-icons" xmlns="http://www.w3.org/2000/svg" version="1.1" xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" preserveAspectRatio="xMinYMin meet"><path class="svga-share-icons-path" fill="'+Z[e]+'" d="'+y[U[e]]+'"/></svg>');for(n in b)if(b.hasOwnProperty(n)){F=b[n].scaleFactor;y=b[n].colors;$("#svga-elements").append('<div class="svga-elements-wrap" id="svga-elements-'+n+'"></div>');for(e=0;e<b[n].shapes.length;e++){$("#svga-elements-"+
n).append('<div class="svga-elements" id="svga-elements-'+n+"-"+e+'" data-zone="'+n+'" data-shape="'+e+'"></div>');var A=SVG("svga-elements-"+n+"-"+e).size("100%","100%").attr({id:"svga-svgcanvas-elements-"+n+"-"+e,width:null,height:null,"class":"svga-svg",viewBox:"0 0 200 200",preserveAspectRatio:"xMinYMin meet"}).group(),u=e,t;for(t in b[n].shapes[u])!b[n].shapes[u].hasOwnProperty(t)||"right"!==t&&"single"!==t&&"back"!==t&&"front"!==t||(b[n].shapes[u][t].length?(m(A,n,u,t),A.scale(F),L=A.bbox(),
A.move(-L.x*F+(200-L.width)/2,-L.y*F+(200-L.height)/2),"clothes"===n&&A.move(10,5),"hair"===n&&0<u&&A.move(0,20)):$("#svga-elements-"+n+"-"+e).empty().append("<div></div>").addClass("empty"))}$("#svga-elements-"+n).hide();$("#svga-colors").append('<div id="svga-colors-'+n+'" class="svga-colors-set"></div>');for(e=0;e<y.length;e++)$("#svga-colors-"+n).append("<div></div>"),$("#svga-colors-"+n+" div:last-child").css("background-color",y[e]);$("#svga-colors-"+n).hide()}$("#svga-custom-color").hide();
for(e=u=0;e<G.length;e++){if("backs"===G[e]||"hair"===G[e])u=1;for(t in b[G[e]].shapes[u])b[G[e]].shapes[u].hasOwnProperty(t)&&(n="svga-group-"+G[e]+"-"+t,$("#"+n).empty(),A=SVG.get(n),H=!0,m(A,G[e],u,t));M[G[e]]=u;u=0}$(".svga-colors-set > div").click(function(){var a=$(this);a.siblings().removeClass("svga-active");a.addClass("svga-active");u=0;a=a.css("background-color");a=tinycolor(a).toHexString();$("#svga-custom-color > input").spectrum("set",a);switch(p){case "faceshape":var c="faceshape humanbody chinshadow facehighlight ears mouth nose eyesfront".split(" ");
E(c,a);break;case "ears":c="faceshape humanbody chinshadow facehighlight ears mouth nose eyesfront".split(" ");E(c,a);break;case "nose":c="faceshape humanbody chinshadow facehighlight ears mouth nose eyesfront".split(" ");E(c,a);break;default:c=[p],E(c,a)}k=++k});$("#svga-custom-color > input").spectrum({color:"#000000",clickoutFiresChange:!0,showInput:!0,showInitial:!0,showButtons:!1,move:V,change:V});$(".sp-replacer").click(function(){$("#svga-colors-"+p+" div.svga-active").removeClass("svga-active");
k=++k});$(".svga-blocks").click(function(){var a=$(this),c=a.data("bodyzones").split(","),b=a.data("blockname");a.siblings().removeClass("svga-active");a.addClass("svga-active");$("#svga-bodyzones").children().hide();for(a=0;a<c.length;a++)$("#svga-bodyzones-"+c[a]).show();$("#svga-bodyzones").children().removeClass("svga-active");$("#svga-bodyzones-"+X[b]).addClass("svga-active").trigger("click")});$(".svga-bodyzones").click(function(){var a=$(this);p=a.data("bodyzone");var c=a.data("block"),d=a.data("controls").split(",");
a.siblings().removeClass("svga-active");a.addClass("svga-active");$("#svga-elements").children().hide();$("#svga-elements-"+p).show();$("#svga-colors").children().hide();$("#svga-custom-color").hide();b[p].colors&&($("#svga-colors-"+p).show(),$("#svga-custom-color").show());$("#svga-controls").children().hide();for(a=0;a<d.length;a++)$("#svga-controls-"+d[a]).css("display","inline-block");X[c]=p;k=--k;$("#svga-elements-"+p+"-"+M[p]).addClass("svga-active-element").trigger("click")});$(".svga-elements").click(function(){for(var a=
$(this),c=a.data(),d=c.zone,d="eyesfront"===d?["eyesback","eyesfront"]:"faceshape"===d?["faceshape","chinshadow"]:d.split(),e=0;e<d.length;e++){$("#svga-custom-color > input").spectrum("set",w[d[e]]);u="facehighlight"===d[e]||"humanbody"===d[e]?0:c.shape;for(t in b[d[e]].shapes[u])if(b[d[e]].shapes[u].hasOwnProperty(t)){var f=SVG.get("svga-group-"+d[e]+"-"+t);$("#svga-group-"+d[e]+"-"+t).empty();H=!0;m(f,d[e],u,t)}a.siblings().removeClass("svga-active-element");a.addClass("svga-active-element");M[d[e]]=
u}k=++k});$(".svga-glob-controls").click(function(){var a=$(this).attr("id"),c=SVG.get("svga-group-subwrapper");switch(a){case "svga-glob-controls-up":c.svgaUp(3,2);break;case "svga-glob-controls-down":c.svgaDown(3,2);break;case "svga-glob-controls-left":c.svgaLeft(4,2);break;case "svga-glob-controls-right":c.svgaRight(4,2);break;case "svga-glob-controls-scaleup":c.svgaScaleUp();break;case "svga-glob-controls-scaledown":c.svgaScaleDown();break;case "svga-glob-controls-tiltleft":c=SVG.get("svga-group-hair-back");
c.svgaRotateLeft(1,3,100,150);c=SVG.get("svga-group-head");c.svgaRotateLeft(1,3,100,150);break;case "svga-glob-controls-tiltright":c=SVG.get("svga-group-hair-back"),c.svgaRotateRight(1,3,100,150),c=SVG.get("svga-group-head"),c.svgaRotateRight(1,3,100,150)}k=++k});$("#svga-controls-up").click(function(){var a;switch(p){case "mouth":a=SVG.get("svga-group-mouth-single");a.svgaUp();break;case "ears":a=SVG.get("svga-group-ears-left");a.svgaUp(1);a=SVG.get("svga-group-ears-right");a.svgaUp(1);break;case "nose":a=
...
SVG.get("svga-group-beard-single");a.svgaUp(4);break;case "glasses":a=SVG.get("svga-group-glasses-single"),a.svgaUp(5)}k=++k});$("#svga-controls-down").click(function(){var a;switch(p){case "mouth":a=SVG.get("svga-group-mouth-single");a.svgaDown();break;case "ears":a=SVG.get("svga-group-ears-left");a.svgaDown(1);a=SVG.get("svga-group-ears-right");a.svgaDown(1);break;case "nose":a=SVG.get("svga-group-nose-single");a.svgaDown(4);break;case "eyebrows":a=SVG.get("svga-group-eyebrows-left");a.svgaDown(4);
...
a.svgaDown(5)}k=++k});$("#svga-controls-left").click(function(){var a;switch(p){case "mouth":a=SVG.get("svga-group-mouth-single");a.svgaLeft(2,0.5);break;case "nose":a=SVG.get("svga-group-nose-single");a.svgaLeft(2,0.5);break;case "eyesiris":a=SVG.get("svga-group-eyesiriscontrol-left");a.svgaLeft();a=SVG.get("svga-group-eyesiriscontrol-right");a.svgaLeft();break;case "mustache":a=SVG.get("svga-group-mustache-single");a.svgaLeft(2,0.5);break;case "beard":a=SVG.get("svga-group-beard-single"),a.svgaLeft(2,
0.5)}k=++k});$("#svga-controls-right").click(function(){var a;switch(p){case "mouth":a=SVG.get("svga-group-mouth-single");a.svgaRight(2,0.5);break;case "nose":a=SVG.get("svga-group-nose-single");a.svgaRight(2,0.5);break;case "eyesiris":a=SVG.get("svga-group-eyesiriscontrol-left");a.svgaRight();a=SVG.get("svga-group-eyesiriscontrol-right");a.svgaRight();break;case "mustache":a=SVG.get("svga-group-mustache-single");a.svgaRight(2,0.5);break;case "beard":a=SVG.get("svga-group-beard-single"),a.svgaRight(2,
0.5)}k=++k});$("#svga-controls-scaleup").click(function(){var a;switch(p){case "faceshape":a=SVG.get("svga-group-faceshape-wrap");a.svgaScaleUp(2,0.02,1E-4);a=SVG.get("svga-group-hair-back");a.svgaScaleUp(2,0.02,1E-4);a=SVG.get("svga-group-hair-front");a.svgaScaleUp(2,0.02,1E-4);"boys"===f&&(a=SVG.get("svga-group-beardwrap"),a.svgaScaleUp(2,0.02,1E-4));a=SVG.get("svga-group-ears-left");a.svgaLeft(2,1.5);a=SVG.get("svga-group-ears-right");a.svgaRight(2,1.5);break;case "mouth":a=SVG.get("svga-group-mouth-single");
...
break;case "beard":a=SVG.get("svga-group-beard-single");a.svgaScaleDown(3);break;case "glasses":a=SVG.get("svga-group-glasses-single"),a.svgaScaleDown(3)}k=++k});$("#svga-controls-tightly").click(function(){var a;switch(p){case "eyebrows":a=SVG.get("svga-group-eyebrows-left");a.svgaRight();a=SVG.get("svga-group-eyebrows-right");a.svgaLeft();break;case "eyesfront":a=SVG.get("svga-group-eyes-left"),a.svgaRight(),a=SVG.get("svga-group-eyes-right"),a.svgaLeft()}k=++k});$("#svga-controls-wider").click(function(){var a;
switch(p){case "eyebrows":a=SVG.get("svga-group-eyebrows-left");a.svgaLeft();a=SVG.get("svga-group-eyebrows-right");a.svgaRight();break;case "eyesfront":a=SVG.get("svga-group-eyes-left"),a.svgaLeft(),a=SVG.get("svga-group-eyes-right"),a.svgaRight()}k=++k});$("#svga-controls-eb1").click(function(){var a;a=SVG.get("svga-group-eyebrows-left");a.svgaCancelRotate().svgaRotateRight(1,rotate_step/2);a=SVG.get("svga-group-eyebrows-right");a.svgaCancelRotate().svgaRotateRight(1,rotate_step/4);k=++k});$("#svga-controls-eb2").click(function(){var a;
a=SVG.get("svga-group-eyebrows-left");a.svgaCancelRotate().svgaRotateLeft(1,rotate_step/4);a=SVG.get("svga-group-eyebrows-right");a.svgaCancelRotate().svgaRotateLeft(1,rotate_step/2);k=++k});$("#svga-controls-eb3").click(function(){var a;a=SVG.get("svga-group-eyebrows-left");a.svgaCancelRotate().svgaRotateRight(1,rotate_step/2);a=SVG.get("svga-group-eyebrows-right");a.svgaCancelRotate().svgaRotateLeft(1,rotate_step/2);k=++k});$("#svga-controls-eb4").click(function(){var a;a=SVG.get("svga-group-eyebrows-left");
a.svgaCancelRotate().svgaRotateLeft(1,rotate_step/1.4);a=SVG.get("svga-group-eyebrows-right");a.svgaCancelRotate().svgaRotateRight(1,rotate_step/1.4);k=++k});$("#svga-controls-ebcancel").click(function(){var a;a=SVG.get("svga-group-eyebrows-left");a.svgaCancelRotate();a=SVG.get("svga-group-eyebrows-right");a.svgaCancelRotate()});$("#svga-resetavatar").click(function(){2>=k&&!P?S():(R="reset",$("#svga-blocks-face").trigger("click"),$("#svga-bodyzones-faceshape").trigger("click"),$("#svga-work-overlay").fadeIn("fast"),
$("#svga-dialog").fadeIn("fast"))});$("#svga-randomavatar").click(function(){2>=k?W():(R="random",$("#svga-work-overlay").fadeIn("fast"),$("#svga-dialog").fadeIn("fast"))});$("#svga-dialog-ok").click(function(){"reset"===R?(S(),$("#svga-work-overlay").fadeOut("fast"),$("#svga-dialog").fadeOut("fast")):"random"===R&&(W(),$("#svga-work-overlay").fadeOut("fast"),$("#svga-dialog").fadeOut("fast"))});$("#svga-dialog-cancel").click(function(){$("#svga-work-overlay").fadeOut("fast");$("#svga-dialog").fadeOut("fast")});
...
" ");if("png"===save_format){var c=document.getElementById("svga-canvas");canvg(c,a,{ignoreMouse:!0,ignoreDimensions:!0});var c=c.toDataURL("image/png"),b="svgA"+(Math.random()+"").replace("0.","")+".png"}else"svg"===save_format&&(c=a,b="svgA"+(Math.random()+"").replace("0.","")+".svg");$("#svga-canvas").attr({width:200,height:200});$.ajax({url:path_to_folder+"php/save-ready-avatar.php",type:"post",dataType:"text",data:{imgdata:c,filename:b},cache:!1,success:function(a){"saved"==a?($("#svga-message-text").html(alert_success).removeClass("svga-error"),
$("#svga-work-overlay").fadeIn("fast")):($("#svga-work-overlay").fadeIn("fast"),$("#svga-message-text").html(alert_error).addClass("svga-error"));$("#svga-message").fadeIn("fast")},error:function(){$("#svga-message-text").html(alert_error).addClass("svga-error");$("#svga-work-overlay").fadeIn("fast");$("#svga-message").fadeIn("fast")}})});$("#svga-gravataravatar").click(function(){$("#svga-work-overlay").fadeIn("fast");$("#svga-gravatar").fadeIn("fast")});$("#svga-gravatar-cancel").click(function(){$("#svga-gravatar").hide();
$("#svga-work-overlay").fadeOut("fast")});$("#svga-gravatar-ok").click(function(){$("#svga-gravatar").hide();$("#svga-loader").show();$("#svga-canvas").attr({width:gravatar_size,height:gravatar_size});s&&s.hide();q&&q.hide();var a=$("#svga-svgmain").html(),a='<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="'+gravatar_size+'px" height="'+gravatar_size+'px" xlink="http://www.w3.org/1999/xlink" style="overflow:hidden!important;"><g id="svga-group-wrapper" transform="scale('+gravatar_size/
200+" "+gravatar_size/200+')">'+a.replace(/<svg(.*?)>/,"").replace(/<g id(.*?)>/,"").replace(/data-(.*?)\"(.*?)\"/g,"").replace(/   /g," ").replace(/  /g," ").replace(/  /g," "),c=document.getElementById("svga-canvas");canvg(c,a,{ignoreMouse:!0,ignoreDimensions:!0});var a=c.toDataURL("image/png"),c=$("#svga-gravatar-email").val(),b=$("#svga-gravatar-pass").val(),d=$("input[name=svga-gr-rating]:checked").val();$("#svga-canvas").attr({width:200,height:200});$.ajax({url:path_to_folder+"php/send-to-gravatar.php",
type:"post",dataType:"text",data:{imgdata:a,datastring1:c,datastring2:b,rating:d},cache:!1,success:function(a){if("success"==a)$("#svga-message-text").removeClass("svga-error"),$("#svga-loader").hide(),$("#svga-message-text").html(alert_success_gravatar),$("#svga-message").fadeIn("fast");else switch(a){case "ratingfail":I(alert_error_ratingfail);break;case "emailfail":I(alert_error_emailfail);break;case "passwordfail":I(alert_error_passwordfail);break;case "imagefail":I(alert_error_imagefail);break;
case "faultcode-8":$("#svga-loader").hide();$("#svga-message-text").html(alert_error_faultcode_8).addClass("svga-error");$("#svga-work-overlay").fadeIn("fast");$("#svga-message").fadeIn("fast");break;case "faultcode-9":I(alert_error_faultcode_9);break;default:I(alert_common_error_gravatar)}},error:function(){I(alert_common_error_gravatar)}})});$("#svga-png-one").click(function(){$("#svga-work-overlay").fadeIn("fast");$("#svga-canvas").attr({width:png_one_download_size,height:png_one_download_size});
...
" "),c=document.getElementById("svga-canvas");canvg(c,a,{ignoreMouse:!0,ignoreDimensions:!0});var a=c.toDataURL("image/png"),b="svgA"+(Math.random()+"").replace("0.","")+".png";$("#svga-canvas").attr({width:200,height:200});$.ajax({url:path_to_folder+"php/save-temp-avatar.php",type:"post",dataType:"text",data:{imgdata:a,filename:b},cache:!1,success:function(a){"saved"==a?($("#svga-message-text").removeClass("svga-error"),Q(b,"png"),$("#svga-work-overlay").fadeOut("fast")):($("#svga-message-text").html(alert_error_download).addClass("svga-error"),
...
png_two_download_size+'px" height="'+png_two_download_size+'px" xlink="http://www.w3.org/1999/xlink" style="overflow:hidden!important;"><g id="svga-group-wrapper" transform="scale('+png_two_download_size/200+" "+png_two_download_size/200+')">'+a.replace(/<svg(.*?)>/,"").replace(/<g id(.*?)>/,"").replace(/data-(.*?)\"(.*?)\"/g,"").replace(/   /g," ").replace(/  /g," ").replace(/  /g," "),c=document.getElementById("svga-canvas");canvg(c,a,{ignoreMouse:!0,ignoreDimensions:!0});var a=c.toDataURL("image/png"),
b="svgA"+(Math.random()+"").replace("0.","")+".png";$("#svga-canvas").attr({width:200,height:200});$.ajax({url:path_to_folder+"php/save-temp-avatar.php",type:"post",dataType:"text",data:{imgdata:a,filename:b},cache:!1,success:function(a){"saved"==a?($("#svga-message-text").removeClass("svga-error"),Q(b,"png"),$("#svga-work-overlay").fadeOut("fast")):($("#svga-message-text").html(alert_error_download).addClass("svga-error"),$("#svga-work-overlay").fadeIn("fast"),$("#svga-message").fadeIn("fast"))},
...
'px" xlink="http://www.w3.org/1999/xlink" style="overflow:hidden!important;"><g id="svga-group-wrapper" transform="scale('+png_ios_download_size/200+" "+png_ios_download_size/200+')">'+a.replace(/<svg(.*?)>/,"").replace(/<g id(.*?)>/,"").replace(/data-(.*?)\"(.*?)\"/g,"").replace(/   /g," ").replace(/  /g," ").replace(/  /g," "),c=document.getElementById("svga-canvas");canvg(c,a,{ignoreMouse:!0,ignoreDimensions:!0});var a=c.toDataURL("image/png"),b="svgA"+(Math.random()+"").replace("0.","")+".png";
$("#svga-canvas").attr({width:200,height:200});$.ajax({url:path_to_folder+"php/save-temp-avatar.php",type:"post",dataType:"text",data:{imgdata:a,filename:b},cache:!1,success:function(a){if("saved"==a){$("#svga-message-text").removeClass("svga-error");var c=$("<img />").error(function(){$(this).hide();$("#svga-message-text").html(alert_error_image).addClass("svga-error");$("#svga-work-overlay").fadeIn("fast");$("#svga-message").fadeIn("fast")}).attr("src",path_to_folder+"temp-avatars/"+b).on("load",
function(){this.complete&&"undefined"!=typeof this.naturalWidth&&0!=this.naturalWidth?($("#svga-ios-image").empty().append(c),$("#svga-work-overlay").fadeIn("fast"),$("#svga-ios").fadeIn("fast")):($("#svga-message-text").html(alert_error_image).addClass("svga-error"),$("#svga-work-overlay").fadeIn("fast"),$("#svga-message").fadeIn("fast"))})}else $("#svga-message-text").html(alert_error_download).addClass("svga-error"),$("#svga-work-overlay").fadeIn("fast"),$("#svga-message").fadeIn("fast")},error:function(){$("#svga-message-text").html(alert_error_download).addClass("svga-error");
...
png_win8tablet_download_size/200+" "+png_win8tablet_download_size/200+')">'+a.replace(/<svg(.*?)>/,"").replace(/<g id(.*?)>/,"").replace(/data-(.*?)\"(.*?)\"/g,"").replace(/   /g," ").replace(/  /g," ").replace(/  /g," "),c=document.getElementById("svga-canvas");canvg(c,a,{ignoreMouse:!0,ignoreDimensions:!0});var a=c.toDataURL("image/png"),b="svgA"+(Math.random()+"").replace("0.","")+".png";$("#svga-canvas").attr({width:200,height:200});$.ajax({url:path_to_folder+"php/save-temp-avatar.php",type:"post",
dataType:"text",data:{imgdata:a,filename:b},cache:!1,success:function(a){"saved"==a?($("#svga-message-text").removeClass("svga-error"),Q(b,"png"),$("#svga-work-overlay").fadeOut("fast")):($("#svga-message-text").html(alert_error_download).addClass("svga-error"),$("#svga-work-overlay").fadeIn("fast"),$("#svga-message").fadeIn("fast"))},error:function(){$("#svga-message-text").html(alert_error_download).addClass("svga-error");$("#svga-work-overlay").fadeIn("fast");$("#svga-message").fadeIn("fast")}})});
...
"").replace(/   /g," ").replace(/  /g," ").replace(/  /g," "),c="svgA"+(Math.random()+"").replace("0.","")+".svg";$.ajax({url:path_to_folder+"php/save-temp-avatar.php",type:"post",dataType:"text",data:{imgdata:a,filename:c},cache:!1,success:function(a){"saved"==a?($("#svga-message-text").removeClass("svga-error"),Q(c,"svg"),$("#svga-work-overlay").fadeOut("fast")):($("#svga-message-text").html(alert_error_download).addClass("svga-error"),$("#svga-work-overlay").fadeIn("fast"),$("#svga-message").fadeIn("fast"))},
...
share_image_size/200+" "+share_image_size/200+')">'+a.replace(/<svg(.*?)>/,"").replace(/<g id(.*?)>/,"").replace(/data-(.*?)\"(.*?)\"/g,"").replace(/   /g," ").replace(/  /g," ").replace(/  /g," "),c=document.getElementById("svga-canvas");canvg(c,a,{ignoreMouse:!0,ignoreDimensions:!0});var a=c.toDataURL("image/png"),b="svgA"+(Math.random()+"").replace("0.","")+".png";$("#svga-canvas").attr({width:200,height:200});s&&s.hide();q&&q.hide();$.ajax({url:path_to_folder+"php/save-temp-avatar.php",type:"post",
dataType:"text",data:{imgdata:a,filename:b},cache:!1,success:function(a){if("saved"==a){$("#svga-message-text").removeClass("svga-error");var c=$("<img />").error(function(){$(this).hide();$("#svga-message-text").html(alert_error_image).addClass("svga-error");$("#svga-work-overlay").fadeIn("fast");$("#svga-message").fadeIn("fast")}).attr("src",path_to_folder+"temp-avatars/"+b).on("load",function(){this.complete&&"undefined"!=typeof this.naturalWidth&&0!=this.naturalWidth?($("#svga-share-image").empty().append(c),
...
$("#svga-elements-faceshape-"+M.faceshape).trigger("click").addClass("svga-active-element");$("#svga-colors-faceshape > div:nth-child(1)").trigger("click");$("#svga-loader").hide();k=2;P=!1;equalHeight()}function equalHeight(){var f=$(".svga-col-left"),b=$(".svga-col-right");f.height("auto");b.height("auto");if(481<=window.innerWidth){var g=f.height(),m=b.height();g>=m?b.height(g):f.height(m)}}$(window).resize(function(){equalHeight()});$(".scrollbar").scrollbar({showArrows:!1,ignoreMobile:!1});});